...
public class SuppliersTableController implements Initializable {
    ...
    @FXML
    private TableView<Suppliers> CooperatorsTableView;

    @FXML
    private TableColumn<Suppliers, Number> TierTableColumn;

    @FXML
    private TableColumn<Suppliers, String> FirstNameTableColumn;

    @FXML
    private TableColumn<Suppliers, String> LastNameTableColumn;

    @FXML
    private TableColumn<Suppliers, String> TelephoneTableColumn;

    public static Suppliers selectionSupplier;
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        TierTableColumn.setCellValueFactory(column -> new ReadOnlyObjectWrapper<>(CooperatorsTableView.getItems().indexOf(column.getValue()) + 1));
        ...
        CooperatorsTableView.setItems(LoginController.user.getSuppliers(Start.database));
        CooperatorsTableView.setRowFactory(tv -> {
            TableRow<Suppliers> row = new TableRow<>();
            row.setOnMouseClicked(event -> {
                if (event.getClickCount() == 2 && (!row.isEmpty())) {
                    try {
                        selectionSupplier = row.getItem();
                        GoToPage("SupplierInformationDetails", 600, 476);
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                }
    ...
    @FXML
    void CreateNewSupplierButtonAction() throws IOException {
        GoToPage("AddSupplier", 600, 476);
    }
