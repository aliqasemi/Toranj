...
public class ProductsTableController implements Initializable {
    ...
    @FXML
    private TableView<Productes> ProductsTableView;

    @FXML
    private TableColumn<Productes, Number> TierTableColumn;

    @FXML
    private TableColumn<Productes, String> ProductNameTableColumn;

    @FXML
    private TableColumn<Productes, Integer> ProductCountTableColumn;

    @FXML
    private TableColumn<Productes, Double> PriceTableColumn;

    public static Productes selectionProduct;
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        TierTableColumn.setCellValueFactory(column -> new ReadOnlyObjectWrapper<>(ProductsTableView.getItems().indexOf(column.getValue()) + 1));
        ProductNameTableColumn.setCellValueFactory(new PropertyValueFactory<>("name"));
        ProductCountTableColumn.setCellValueFactory(new PropertyValueFactory<>("count"));
        PriceTableColumn.setCellValueFactory(new PropertyValueFactory<>("totalAmont"));
        ProductsTableView.setItems(LoginController.user.getProducts(Start.database));
        ProductsTableView.setRowFactory(tv -> {
            TableRow<Productes> row = new TableRow<>();
            row.setOnMouseClicked(event -> {
                if (event.getClickCount() == 2 && (!row.isEmpty())) {
                    try {
                        selectionProduct = row.getItem();
                        GoToPage("ProductInformationDetails", 600, 411);
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                }
    ...
    @FXML
    void CreateNewProductButtonAction() throws IOException {
